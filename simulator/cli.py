...
def main():
 ...
 response={t:evaluate_mode_response(shares,{m:p.get(m,{}).get(t,0) for m in shares},{m:a.get(m,{}).get(t,0) for m in shares},d.get('match',{}).get(t,{m:0 for m in shares})) for t in d['capacity']}
 out={'revisions':{'config':d.get('configRevision','CFG-DEMO'),'rule':d.get('ruleRevision','RULE-DEMO'),'environment':rev,'bake':d.get('bakeRevision','BAKE-DEMO')},'supply':supply,'qualityShare':quality_share,'shares':shares,'fallback':fb,'distribution':dist,'envWeight':env_weight,'conservation':sum(env_weight.values()),'pFallback':pfb,'channels':channels,'modeResponse':{t:v[0] for t,v in response.items()},'trace':{'fallbackReason':pfb or fb,'targetCount':len(dist)}}
 if args.html:
